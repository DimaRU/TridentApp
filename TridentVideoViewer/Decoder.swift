...
public class PayloadDecoder: NSObject {
    ...
    @objc public convenience init(topicName: UnsafePointer<UInt8>, dataType: UnsafePointer<UInt8>) {
        self.init()
        self.topicName = String(cString: topicName)
        self.dataType = String(cString: dataType)
    }
